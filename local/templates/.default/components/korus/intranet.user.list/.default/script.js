;(function () {
    ...
    namespace.Manager.prototype = {
        ...
        addTask: function (userId) {
            BX.SidePanel.Instance.open('/company/personal/user/' + userId + '/tasks/task/edit/0/?RESPONSIBLE_ID=' + userId + '&ta_sec=user&ta_el=context_menu');
        },
    ...
    document.querySelectorAll('.main-grid-cell-content > a').forEach(function (el) {
        if (!el.className) {
            el.className = 'ui-link ui-link-dark';
        }
    });

    document.querySelectorAll('.main-grid-cell-content > .field-wrap > .field-item > .uf-employee-wrap').forEach(function (el) {
        el.classList.add('ui-link', 'ui-link-dark');
    });

    // BX.ready(() => {
    //     const intranetUserListScrollbar = document.querySelector('.main-grid-container');
    //     intranetUserListScrollbar && BX.KTeam.Scrollbar.init(intranetUserListScrollbar);
    // });
})();
