...
<div class="search-page">
    <form action="" method="get" name="sonet_content_search_form" id="sonet_content_search_form">
            ...
            if (!in_array($key, $params_to_kill)) {
                if (!is_array($val)) {
                    ...
                    foreach ($val as $value) {
                        ?>
                        <input type="hidden" name="<?= htmlspecialcharsex($key) ?>[]"
        ...
        <table class="sonet-query" cellspacing="0" cellpadding="0" border="0">
            <tr>
                <td style="width: 100%;"><input type="text" class="search-query" name="q"
                                                value="<?= $arResult["REQUEST"]["QUERY"] ?>"/></td>
                <td>&nbsp;</td>
        ...
        <? if (
            ...
            ?>
            <div class="sonet-search-advanced">
            <div class="sonet-search-advanced-filter"><a href="#"
                                                         onclick="document.getElementById('sonet_content_search_filter').style.display = document.getElementById('sonet_content_search_filter').style.display == 'none' ? 'block' : 'none'; return false;"><?= GetMessage("SEARCH_ADDITIONAL_FILTER") ?></a>
            </div></div><?
            ...
            <table class="sonet-search-filter" cellpadding="0" cellspacing="0">
                ...
                if (array_key_exists("DROPDOWN_SONET", $arResult) && is_array($arResult["DROPDOWN_SONET"]) && count($arResult["DROPDOWN_SONET"]) > 0):?>
                    <tr>
                        ...
                        <td class="sonet-search-filter-field"><select name="<?= $arParams["FILTER_NAME"] ?>"
                                                                      class="select-field">
                                <option value=""><?= GetMessage("SEARCH_ALL") ?></option>
                ...
                ?>
                <tr>
                    <td colspan="2" class="sonet-search-filter-field"><input type="submit"
                                                                             clas="sonet-search-filter-button"
