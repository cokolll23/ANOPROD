...
foreach ($arResult["SEARCH"] as $i => $arItem) {
    ...
    switch ($arItem["MODULE_ID"]) {
                        ...
                        foreach ($arElement["SECTION_PATH"] as $arPath) {
                            if ($k > 0) {
                                ...
                                $arPath["URL"] = CComponentEngine::MakePathFromTemplate($arParams["~PATH_TO_USER_" . mb_strtoupper($element_type) . "_SECTION"], array(
                                    ...
                                    "user_alias" => "user_" . $arParams["SOCNET_USER_ID"],
                                ));
        ...
        case "forum":
            ...
            $arChainBody[] = '<a href="' . CComponentEngine::MakePathFromTemplate($arParams["~PATH_TO_USER_FORUM"], array("user_id" => $arParams["SOCNET_USER_ID"])) . '">' . $arFeaturesTitles["forum"] . '</a>';
            $arResult["SEARCH"][$i]["CHAIN_PATH"] = implode('&nbsp;/&nbsp;', $arChainBody);
