...
$db_res = $arParams["OBJECT"]->_get_mixed_list(null, $arParams + array("SHOW_VERSION" => "Y"),
...
if ($arParams["SET_NAV_CHAIN"] != "N") {
    $arResult["NAV_CHAIN"] = $arParams["OBJECT"]->GetNavChain(array(
        "element_id" => ($arResult["ELEMENT"]["WF_PARENT_ELEMENT_ID"] > 0 ?
            $arResult["ELEMENT"]["WF_PARENT_ELEMENT_ID"] : $arResult["ELEMENT"]["ID"])
    ), "array");
...
?><? $APPLICATION->IncludeComponent("bitrix:bizproc.document.history", "", array(
    ...
    "DOCUMENT_URL" => str_replace(
        ...
        array($arResult["VARIABLES"]["ELEMENT_ID"], "#ID#", "#NAME#"),
        $arResult["~PATH_TO_USER_FILES_WEBDAV_BIZPROC_HISTORY_GET"]),
