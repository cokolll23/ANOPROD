                ...
                if ($DISPLAY_VALUE):
                    foreach ($arResult["VOTE_NAMES"] as $i => $name):
                        if (round($DISPLAY_VALUE) > $i):
                            ?>
                            <th>
                            <div id="vote_<? echo $arResult["ID"] ?>_<? echo $i ?>" class="star-voted"></div></th><?
                        else:
                            ?>
                            <th>
                            <div id="vote_<? echo $arResult["ID"] ?>_<? echo $i ?>" class="star-empty"></div></th><?
                        endif;
                    endforeach;
                else:
                    foreach ($arResult["VOTE_NAMES"] as $i => $name):
                        ?>
                        <th>
                        <div id="vote_<? echo $arResult["ID"] ?>_<? echo $i ?>" class="star"></div></th><?
                    endforeach;
                endif;
            else:
                if ($DISPLAY_VALUE):
                    foreach ($arResult["VOTE_NAMES"] as $i => $name):
                        if (round($DISPLAY_VALUE) > $i):
                            ?>
                            <th>
                            <div id="vote_<? echo $arResult["ID"] ?>_<? echo $i ?>" class="star-active star-voted"
                                 title="<? echo $name ?>" <?
                                 ?>onmouseover="if (voteScript) {voteScript.trace_vote(this, true);}" <?
                                 ?>onmouseout="if (voteScript) {voteScript.trace_vote(this, false);}" <?
                                 ?>onclick="if (voteScript) {voteScript.do_vote(this, 'vote_<? echo $arResult["ID"] ?>', <? echo $arResult["AJAX_PARAMS"] ?>);}"></div>
                            </th><?
                        else:
                            ?>
                            <th>
                            <div id="vote_<? echo $arResult["ID"] ?>_<? echo $i ?>" class="star-active star-empty"
                                 title="<? echo $name ?>" <?
                                 ?>onmouseover="if (voteScript) {voteScript.trace_vote(this, true);}" <?
                                 ?>onmouseout="if (voteScript) {voteScript.trace_vote(this, false);}" <?
                                 ?>onclick="if (voteScript) {voteScript.do_vote(this, 'vote_<? echo $arResult["ID"] ?>', <? echo $arResult["AJAX_PARAMS"] ?>);}"></div>
