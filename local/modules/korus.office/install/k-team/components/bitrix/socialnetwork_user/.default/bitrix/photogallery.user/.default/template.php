...
if ($GLOBALS["USER"]->IsAuthorized() && !empty($arResult["GALLERY"])):
    ...
    <a href="<?= ($bIsMine ? $arResult["MY_GALLERY"]["LINK"]["VIEW"] : $arResult["GALLERY"]["LINK"]["VIEW"]) ?>"
       class="photo-action gallery-view" title="<?=
    ...
    if ($arParams["PERMISSION"] >= "W"):
        if ($bIsMine && (count($arResult["MY_GALLERIES"]) > 1 || $arResult["I"]["ACTIONS"]["CREATE_GALLERY"] == "Y")):
            ?>
            <a href="<?= $arResult["LINK"]["GALLERIES"] ?>" class="photo-action gallery-view-list"
               title="<?= GetMessage("P_GALLERIES_MY_TITLE") ?>"><?=
        ...
        ?><a
        href="<?= ($bIsMine ? $arResult["MY_GALLERY"]["LINK"]["UPLOAD"] : $arResult["GALLERY"]["LINK"]["UPLOAD"]) ?>"
        class="photo-action photo-upload"><?= GetMessage("P_UPLOAD") ?></a><?
    endif;
    ?></div>
    </div>
...
if (!empty($arResult["GALLERY"])):
    ?>
    <div
    class="photo-user <?= ($arResult["GALLERY"]["CREATED_BY"] == $GLOBALS["USER"]->GetId() ? " photo-user-my" : "") ?>">
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="gallery-table-header">
        <tr>
            <td width="0%" class="picture">
